...
function renderMonShop(){
  ...
  el.innerHTML=`
    <div class="card" style="margin-bottom:20px"><div class="card-body">
      ...
      <button onclick="toggleShopOpen()" style="padding:8px 20px;border-radius:8px;border:1px solid ${toggleColor};background:transparent;color:${toggleColor};font-family:var(--font-b);font-size:.72rem;font-weight:600;cursor:pointer;transition:background-color .2s,border-color .2s,color .2s,opacity .2s">${toggleLabel}</button>
    </div></div>
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:12px">
        ${entries.map(([item_id, si])=>{
          ...
          return `<div class="shop-item-card rarity-${rarity}" style="position:relative">
            ...
            <button onclick="removeFromShop('${item_id}')" style="margin-top:8px;width:100%;padding:6px;border-radius:6px;border:1px solid var(--red);background:rgba(255,71,87,0.08);color:var(--red);font-family:var(--font-b);font-size:.65rem;font-weight:600;cursor:pointer;transition:background-color .2s,border-color .2s,color .2s,opacity .2s">✕ Retirer de la vente</button>
          </div>`;
        }).join('')}
      </div>
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:12px">
        ${sellable.map(([item_id, qty])=>{
          ...
          return `<div class="shop-item-card rarity-${rarity}" style="position:relative">
            ...
            <button onclick="addToShop('${item_id}',${qty})" style="margin-top:6px;width:100%;padding:6px;border-radius:6px;border:1px solid var(--green);background:rgba(52,211,153,0.08);color:var(--green);font-family:var(--font-b);font-size:.65rem;font-weight:600;cursor:pointer;transition:background-color .2s,border-color .2s,color .2s,opacity .2s">🏷️ Mettre en vente</button>
          </div>`;
        }).join('')}
...
function _injectIRPFilterButton(){
  ...
  btn.id='ucatbtn-irp';
  btn.style.cssText='border:1px solid rgba(220,20,60,0.3);color:#dc143c;background:rgba(220,20,60,0.06);font-family:var(--font-h);font-size:0.5rem;letter-spacing:0.1em;padding:6px 14px;border-radius:6px;cursor:pointer;transition:background-color 0.2s,border-color 0.2s,color 0.2s,opacity 0.2s;margin-left:6px;font-weight:700;';
