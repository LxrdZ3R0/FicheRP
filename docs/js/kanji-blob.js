...
(function () {
  ...
  function mkS(type,src){
    ...
    if(!gl.getShaderParameter(sh,gl.COMPILE_STATUS)){window._dbg?.error('[GLSL compile]',gl.getShaderInfoLog(sh));return null;}
    return sh;
  }
  function mkP(vs,fs){
    ...
    gl.linkProgram(pr);
    if(!gl.getProgramParameter(pr,gl.LINK_STATUS)){window._dbg?.error('[GLSL link]',gl.getProgramInfoLog(pr));return null;}
    return pr;
  }
  function mkFB(cw,ch){
