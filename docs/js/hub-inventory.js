...
function renderCharacterPanel(){
  ...
  slotOrder.forEach(slotId=>{
    ...
    for(let i=0;i<def.max;i++){
      ...
      if(itemId){
        html+=`<div class="slot-cell occupied" data-item-id="${itemId}" data-slot="${slotId}" onclick="showItemDetail('${itemId}')" title="${e(it.name||itemId)}">${it.image?`<img src="${it.image}" alt="${e(it.name||itemId)}" class="slot-cell-img">`:(it.emoji||'📦')}</div>`;
      } else {
  ...
  if(dataChanged){
    ...
    entries.forEach(([id,qty])=>{
      ...
      div.innerHTML=
        ...
        '<span class="inv-item-emoji">'+(it.image?'<img src="'+it.image+'" alt="'+e(it.name||id)+'" class="inv-item-img">':(it.emoji||'📦'))+'</span>'+
...
function showItemDetail(itemId,animate=true){
  ...
  document.getElementById('inv-detail-content').innerHTML=`
    ${it.image?`<img src="${it.image}" alt="${e(it.name||itemId)}" class="inv-detail-img" style="width:64px;height:64px;object-fit:contain;border-radius:8px;margin-bottom:8px">`:`<span class="inv-detail-emoji" style="color:${rc}">${it.emoji||'📦'}</span>`}
